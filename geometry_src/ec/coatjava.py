"""coatjava bridge and volume table parser for the EC geometry."""
...
class CoatjavaVolume:
    """One parsed row from the coatjava EC volume table."""
...
def run_factory(ec_dir, variation, run_number):
    """Run the local coatjava factory and return the generated volume table path."""
    ec_path = Path(ec_dir)
    output = ec_path / f"ec__volumes_{variation}.txt"
    classpath = coatjava_classpath(ec_path)
    compiled_factory = compiled_factory_dir(ec_path)
    if compiled_factory:
        command = [java_command(), "-cp", classpath + os.pathsep + compiled_factory, "CoatjavaFactory"]
    else:
        factory_source = str(ec_path.parent / "coatjava_factories" / "CoatjavaFactory.java")
        command = [java_command(), "-cp", classpath, factory_source]

    command.extend(["--system", "ec", "--variation", variation, "--runnumber", str(run_number)])
    subprocess.run(command, cwd=ec_path, check=True)
    return output
...
def generate_volumes(ec_dir, variation, run_number):
    """Run coatjava, parse the generated volume table, and remove the temporary file."""
    volume_table = run_factory(ec_dir, variation, run_number)
