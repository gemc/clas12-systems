...
bool FTHODODigitization::loadTTImpl([[maybe_unused]] int runno,
    ...
    if (!clas12ccdb::loadTable(calib.get(), "/daq/tt/fthodo:1", data, log)) return false;
    ...
    for (const auto& row : data) {
        ...
        const int component = static_cast<int>(row[5]);
        const int order = static_cast<int>(row[6]);

        table->addGElectronicWithIdentity({sector, layer, component, order},
                                          GElectronic(crate, slot, channel,
                                                      GElectronic::ComparisonMode::crate));
    }

    translationTable = table;
    log->info(1, " FTHODO translation table loaded: ", data.size(), " entries");
    return true;
}
