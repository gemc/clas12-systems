...
std::unique_ptr<GDigitizedData> FTOF_digitization::digitizeHitImpl(GHit* ghit, size_t hitn) {
    ...
    // FTOF thresholds and inefficiencies are optional GEMC2 policies gated by
    // -applyThresholds / -applyInefficiencies. Cache only the deterministic threshold input
    // here; efficiency random draws belong to the dedicated apply_efficiency_impl() hook.
    digitizedData->includeTransientVariable(FTOF_ATTENUATED_EDEP, energyDepositedAttenuated);
