...
# Placement in the YAML: the CAD volumes are daughters of the sector-3 gas mother ltccS3, which geometry.py
# gives a wide triangular wedge-trap to contain them. ltccS3 sits at world (0,0,1973) mm, rotated 0,0,-30 deg
# (passive).
# Re-parenting the mesh-build world placements (walls rot 180,0,60; nose 180,0,120; both at world 0,0,1273.7)
# into ltccS3 keeps their world position and gives these local placements (rotD = rotR * rotM^-1,
# TD = rotM * (TR - TM)); the cones already carry their own ltccS3-local placement.
CAD_MOTHER = "ltccS3"
CAD_POSITION = "0*mm, 0*mm, -699.3*mm"
WALL_YAML_ROTATION = "180*deg, 0*deg, 30*deg"
NOSE_YAML_ROTATION = "180*deg, 0*deg, 90*deg"
...
NOSE_CLEARANCE = 2.0                    # mm; the subtracted walls are dilated by this so the nose sits off
...
def emit_yaml(path):
    lines = [
        "# LTCC CAD mesh sources for GEMC3's gcad feature (see ../../../../src/examples/basic/cad).",
        "# Generated once by `build_cad_meshes.py --yaml`; copy placements are generated by geometry.py.",
    ...
    # merged sector-3 frame assemblies, daughters of the ltccS3 gas wedge (see CAD_* / geometry.py).
    for wall, spec in WALLS.items():
        vol(name=wall, mother=CAD_MOTHER, material="G4_Al", color=spec["color"],
            position=CAD_POSITION, rotation=WALL_YAML_ROTATION)
    # mirror-image walls: reflected mesh, same placement as their source wall.
    for wall, spec in MIRROR_WALLS.items():
        vol(name=wall, mother=CAD_MOTHER, material="G4_Al", color=spec["color"],
            position=CAD_POSITION, rotation=WALL_YAML_ROTATION)
    # nose: single filled solid of the five support pieces, homogenised steel+epoxy material.
    vol(name="nose", mother=CAD_MOTHER, material=NOSE_MATERIAL, color=NOSE_COLOR,
        position=CAD_POSITION, rotation=NOSE_YAML_ROTATION)
    # TEMPORARY (comparison): the raw nose parts, green, same placement so they overlay the nose.
    for name, *_ in INDIVIDUALS:
        vol(name=name, mother=CAD_MOTHER, material="G4_Al", color="00ee00",
            position=CAD_POSITION, rotation=NOSE_YAML_ROTATION)
