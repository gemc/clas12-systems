...
for variation, run in all_runs.items():
    ...
    define_mirrors(cfg)
    build_ltcc(cfg)
    # The mixed native/CAD rows share the LTCC system, variation and run. GEMC selects the Geant4
    # object builder from each row's solid type, so one sqlite gsystem loads the complete detector.
    upload_cad_definitions(cad_yaml, cfg.dbhost, experiment="clas12", variation=variation,
                           run=run, verbosity=0)
    build_cad_copies(cfg)
