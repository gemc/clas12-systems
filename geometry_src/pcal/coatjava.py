"""coatjava bridge and volume table parser for the PCAL geometry."""
...
class CoatjavaVolume:
    """One parsed row from the coatjava PCAL volume table."""
...
def run_factory(pcal_dir, variation, run_number):
    ...
    if compiled_factory:
        ...
        factory_source = str(pcal_path.parent / "coatjava_factories" / "CoatjavaFactory.java")
        command = [java_command(), "-cp", classpath, factory_source]

    command.extend(["--system", "pcal", "--variation", variation, "--runnumber", str(run_number)])
    subprocess.run(command, cwd=pcal_path, check=True)
    return output
...
def generate_volumes(pcal_dir, variation, run_number):
    """Run coatjava, parse the generated volume table, and remove the temporary file."""
    volume_table = run_factory(pcal_dir, variation, run_number)
